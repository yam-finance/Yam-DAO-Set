...
contract HEVMHelpers is DSTest {
            ...
            if (ins.length > 0) {
                for (uint256 j = 0; j < ins.length; j++) {
                    if (j != 0) {
                        ...
                        slot = keccak256(abi.encode(ins[j], uint256(i)));
                    }
                }
            } else {
    ...
    function write_flat(
        ...
        if (!finds[who][sigs(sig)]) {
            find(sig, ins, who, bytes32(uint256(13371337)));
        }
        writ(sig, ins, 0, who, bytes32(value));
    }
    ...
    function write_flat(
        ...
        writ(sig, ins, 0, who, bytes32(uint256(value)));
    }
    ...
    function write_map(
        ...
        if (!finds[who][sigs(sig)]) {
            find(sig, keys, who, bytes32(uint256(13371337)));
        }
        writ(sig, keys, 0, who, bytes32(value));
    }
    ...
    function write_map(
        ...
        writ(sig, keys, 0, who, bytes32(uint256(value)));
    }
    ...
    function write_deep_map_struct(
        address who,
        string memory sig,
        bytes32[] memory keys,
        uint256 value,
        ...
        writ(sig, keys, depth, who, bytes32(value));
    }
    ...
    function write_deep_map_struct(
        ...
        writ(sig, keys, depth, who, bytes32(uint256(value)));
    }
...
contract TreasuryHelpers is HEVMHelpers {
